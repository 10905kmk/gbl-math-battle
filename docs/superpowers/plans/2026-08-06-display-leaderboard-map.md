...
Opus 모델로 전체 diff를 리뷰했다(Playwright 없이 코드 읽기 + 실제 회귀 테스트 + 직접 작성한 확인용 스크립트로). 발견된 이슈를 모두 수정했다(커밋: `fix: opus 리뷰에서 발견된 Important 이슈 수정`):

- **Important I1**: socket.io는 재연결 시 새 `socket.id`를 발급하는데, 서버가 이름을 `socket.id` 기준(`participantNames` Map)으로만 들고 있어서 와이파이가 잠깐 끊겼다 붙는 것만으로 참가자 이름이 영구히 사라졌다(로컬 `name` 상태는 그대로라 이름 화면이 다시 안 뜨는데, 서버는 새 id에 대해 이름을 모르는 상태) — 실제 재현 확인됨. `nameRef`로 이름을 들고 있다가 매 재연결(`participant:join`과 같은 타이밍, 지연 없이)마다 `participant:name`을 다시 보내도록 수정.
- **Important I2**: 스펙에 "순위/색상 원/이름/점수"라고 명시했는데 실제 구현엔 순위 번호가 빠져 있었다(계획 작성 시 누락) — `leaderboard-rank` 추가.
- **Minor(반영)**: `participant:name`이 `create:done`보다 늦게 도착하는 경우(재연결 등) 이미 만들어진 `cohort.participants` 엔트리에도 즉시 반영, 리더보드 정렬 비교식에 `score ?? 0` 방어(하나라도 `undefined`면 비교가 `NaN`이 되어 전체 순서가 조용히 뒤섞이는 걸 방지 — 지금은 발생 안 하지만 이 프로젝트 전반의 이중 방어 원칙), `onState`가 `room.players` 없는 payload도 방어, 연결 끊긴 참가자를 미니맵처럼 리더보드에서도 흐리게 표시(일관성).
- **보류(수정 안 함)**: 좁은 화면(1366x768 등)에서 공용화면 레이아웃이 넘칠 수 있다는 지적 — 공용화면은 항상 큰 화면(전자칠판/프로젝터)에 띄우는 용도라 우선순위 낮음으로 보류. Konva 노드 정리 안 하는 것(`nodesRef`) — 참가자 화면의 기존 패턴과 동일, 대전 중 플레이어가 안 사라지므로 실질적 문제 없음.

## 라이브 검증 (사용자 직접 확인용)

Playwright 자동 확인 대신 아래를 사용자가 직접 브라우저로 확인한다:

1. 실제 공용화면 해상도(1920x1080 이상 권장)에서 미니맵+리더보드 레이아웃이 잘리지 않는지.
2. 배경 이미지가 960x720으로 축소되어 뜨는지(첫 로드 지연 있을 수 있음).
3. 참가자 4~5명으로 캐릭터 점 위치가 실제 화면과 상대적으로 일치하는지.
4. 이름 넣은/안 넣은 참가자 섞어서 이름/"캐릭터 N" 표시 확인, 점수 변화에 따라 순위가 실시간 재정렬되는지.
5. 참가자 소켓을 강제로 재연결시켜(예: 네트워크 잠깐 끊기) 이름이 유지되는지(I1 수정 확인).
6. 학습→제작→대전→결과 전 구간에서 공용화면 브라우저 콘솔에 에러가 없는지.
